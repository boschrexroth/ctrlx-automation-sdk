...
if [[ "--help" = $1 ]]; then
	...
	echo "and the name of the branch to be created is required."
	echo "Additionally please enter whether aarch64 libs should be installed to be able to cross compile C++ projects."
	echo ""
	exit 1
fi
...
echo "Enter the desired version of the zip archive to install (e.g. 2.2.3)."
echo "This will be used as Tag for the local github repository."
echo $SEPARATION_LINE_2
read -p "Version (release)? " TAG

# TODO Latest

echo ""
echo $SEPARATION_LINE_1
echo "Give a name to the branch to be created in the local Github repository.."
echo $SEPARATION_LINE_2
read -p "Branch name? " BRANCH

echo ""
echo $SEPARATION_LINE_1
echo "Install aarch64 libs to be able to cross compile C++ projects?"
echo $SEPARATION_LINE_2
read -p "Install y/n? " AARCH64LIBS
...
echo "Install aarch64 libs:  ${AARCH64LIBS}"
echo $SEPARATION_LINE_2
read -t 20 -p "OK? "
...
echo ""
echo $SEPARATION_LINE_1
echo "Checking out version (tag) ${TAG} and creating branch ${BRANCH} ..."
echo $SEPARATION_LINE_2
