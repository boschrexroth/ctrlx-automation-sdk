...
from ctrlxdatalayer.bulk import (
    Response,
    BulkReadRequest,
    BulkWriteRequest,
    BulkCreateRequest,
...
__root_node = "sdk/cpp/datalayer/provider/alldata"
...
def cb_read_async(
    responses: List[Response], userdata: ctrlxdatalayer.clib.userData_c_void_p
):
    """cb_read_async"""
    for response in responses:
        if response.get_result() == Result.OK:
            ...
            log.debug(
                "Bulk read: {a} {b} {c}".format(a=dt, b=addr, c=data.get_float64())
            )
...
def main():
    ...
    with ctrlxdatalayer.system.System("") as datalayer_system:
        ...
        if datalayer_client is None:
            log.error(
                "Connecting '{c}' failed".format(c=datalayer_client_connection_string)
            )
            sys.exit(1)
        ...
        with (
            ...
            if datalayer_client.is_connected() is False:
                log.error(
                    "ctrlX Data Layer is NOT connected:"
...
def __bulk_delete(client):
    ...
    with bulk:
        result = bulk.delete(
            [
                __root_node + "/dynamic/py-created/int8",
                __root_node + "/dynamic/py-created/string",
            ]
        )
        if result != Result.OK:
        ...
        for response in bulk.get_response():
            log.debug(
                "Bulk delete: {a} {b}".format(
                    a=response.get_address(), b=response.get_result()
                )
            )
...
def __bulk_create(client):
    ...
    with bulk:
        with Variant() as data:
            data.set_int8(-127)
            req = [BulkCreateRequest(__root_node + "/dynamic/py-created/int8", data)]
            data.set_string("Created by bulk create")
            req.append(
                BulkCreateRequest(__root_node + "/dynamic/py-created/int8", data)
            )
...
def __bulk_metadata(client):
    ...
    with bulk:
        ...
        for response in bulk.get_response():
            log.debug(
                "Bulk metadata: {a} {b}".format(
                    a=response.get_address(), b=response.get_result()
                )
            )
            if response.get_result() != Result.OK:
                log.debug(
                    "Bulk metadata response error: {a}".format(response.get_result())
                )
                continue
            ...
            log.debug(
                "\tallowedOperations read={a} write={b} create={c} delete={d}".format(
                    a=allowedoperations.Read(),
                    b=allowedoperations.Write(),
                    c=allowedoperations.Create(),
                    d=allowedoperations.Delete(),
                )
            )
...
def __bulk_browse(client):
    ...
    with bulk:
        ...
        for response in bulk.get_response():
            log.debug(
                "Bulk browse: {a} {b}".format(
                    a=response.get_address(), b=response.get_result()
                )
            )
            if response.get_result() != Result.OK:
                log.debug(
                    "Bulk browse response error: {a}".format(response.get_result())
                )
                continue
...
def __bulk_write(client):
    ...
    with bulk:
        ...
        req.extend(
            [
                BulkWriteRequest("sdk-py-provider-alldata/dynamic/bool8", data),
                BulkWriteRequest(__root_node + "/dynamic/bool8", data),
            ]
        )
        ...
        req.extend(
            [
                BulkWriteRequest("sdk-py-provider-alldata/dynamic/float32", dataf),
                BulkWriteRequest(__root_node + "/dynamic/float32", dataf),
            ]
        )
        ...
        req.extend(
            [
                BulkWriteRequest("sdk-py-provider-alldata/dynamic/int8", datai),
                BulkWriteRequest(__root_node + "/dynamic/int8", datai),
            ]
        )
        ...
        req.extend(
            [
                BulkWriteRequest("sdk-py-provider-alldata/dynamic/string", datas),
                BulkWriteRequest(__root_node + "/dynamic/string", datas),
            ]
        )
