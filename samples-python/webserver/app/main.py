...
import http.server
import app.web.request_handler
import app.web.unix_socket_server
...
def run():
    ...
    client, connection_string = app.web.request_handler.data_layer.connect_client(
...
def main():
    ...
    while not __close_app:
        app.web.request_handler.data_layer = app.datalayer.DataLayer()
        app.web.request_handler.data_layer.start()

        run()
        time.sleep(10.0)

        app.web.request_handler.data_layer.stop()
...
def run_webserver_tcp():
    ...
    with http.server.HTTPServer(
        ("", httpServerPort), app.web.request_handler.RequestHandler
    ) as http_server:
...
def run_webserver_unixsock():
    ...
    with app.web.unix_socket_server.UnixSocketServer(
        sock_file, app.web.request_handler.RequestHandler
    ) as http_server:
