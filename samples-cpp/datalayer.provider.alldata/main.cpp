...
int main(void)
{
  ...
  for (;;)
  {
    ...
    auto provider = std::shared_ptr<comm::datalayer::IProvider3>(getProvider(datalayerSystem, "10.0.2.2", "boschrexroth", "boschrexroth", 8443));
    ...
    {
      auto providerNodeStatic = std::make_unique<ProviderNodeAllData>(provider, g_dataLayerAddrBase, false);
      providerNodeStatic->registerNodes();

      auto providerNodeDynamic = std::make_unique<ProviderNodeAllData>(provider, g_dataLayerAddrBase, true);
